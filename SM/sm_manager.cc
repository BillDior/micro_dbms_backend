...
RC SM_Manager::CreateDb(const char *dbName) {
  ...
  if (NULL != dir) {
    printf("Failed to create database %s, Please try again after changing the "
           "database name \n",
           dbName);
    // printf("There is a folder with the same name in the current directory. "
    //       "Please try again after changing the database name. \n");
    closedir(dir);
    return rc;
  }
  // create database
  ...
  if (chdir(dbName) < 0) {
    ...
    const char *(attrName[MAXNAME + 1]) = {
        ...
        "recordCount", "relName",     "attrName",          "offset",
        "attrType",    "attrSize",    "attrDisplayLength", "attrSpecs",
        "indexNo"};
  ...
  //是目录
    ...
    while ((dir = readdir(dirp)) != NULL) {
      ...
      if (S_ISREG(st.st_mode)) {
        ...
        printf(
            "The database %s contains illegal files and cannot be deleted. \n",
            dbName);
        continue;
      }
    }
...
RC SM_Manager::CreateTable(const char *relName, int attrCount,
  ...
  for (int i = 0; i < attrCount; ++i) {
    ...
    // + 1 为了终结符 '\0'
    attrEntry.attrSize =
        attributes[i].attrType == STRING ? attributes[i].attrLength + 1 : 4;
  ...
  for (int i = 0; i < attrCount; ++i)
    if (attributes[i].attrSpecs & ATTR_SPEC_PRIMARYKEY)
      TRY(ixm->CreateIndex(relName, relEntry.indexCount++,
                           attributes[i].attrType, attributes[i].attrLength));
...
RC SM_Manager::DropTable(const char *relName) {
  ...
  //检索attrcat文件
  TRY(scan.OpenScan(attrcat, STRING, MAXNAME + 1,
                    offsetof(AttrCatEntry, relName), EQ_OP, (void *)relName));
  RC retcode;
  //没达到文件末尾
...
RC SM_Manager::CreateIndex(const char *relName, const char *attrName) {
  ...
  RM_Record rec;
  //调用创建索引函数
  TRY(ixm->CreateIndex(relName, indexNo, attrEntry->attrType,
                       attrEntry->attrSize));
  //打开主文件和索引文件
...
RC SM_Manager::DropIndex(const char *relName, const char *attrName) {
  ...
  while ((retcode = scan.GetNextRec(attrRec)) != RM_EOF) {
    ...
    if (attrEntry->indexNo > curDeleteIndexNo) {
      ...
      // printf(" old: %s\n new: %s", oldIndexFileName.c_str(),
      // newIndexFileName.c_str());
...
/**
 ...
 输入参数：relName关系名称
         ...
         attrEntry 属性目录实体项
*/
RC SM_Manager::GetAttrEntry(const char *relName, const char *attrName,
                            AttrCatEntry &attrEntry) {
  RM_Record rec;
  char *data;
...
/**
 ...
 输入参数：relName关系名称
         ...
         rec RM_Record实体
*/
RC SM_Manager::GetAttrCatEntry(const char *relName, const char *attrName,
                               RM_Record &rec) {
  AttrCatEntry *attrEntry;
  RM_FileScan scan;
  RID rid;
 ...
 */
RC SM_Manager::UpdateRelEntry(const char *relName,
                              const RelCatEntry &relEntry) {
  RM_Record rec;
  char *data;
  //获取relcat指定项
  TRY(GetRelCatEntry(relName, rec));
...
*/
RC SM_Manager::UpdateAttrEntry(const char *relName, const char *attrName,
                               const AttrCatEntry &attrEntry) {
  RM_Record rec;
  char *data;
  //获取attrcat指定项
  TRY(GetAttrCatEntry(relName, attrName, rec));
...
RC SM_Manager::GetDataAttrInfo(const char *relName, int &attrCount,
  ...
  while ((retcode = scan.GetNextRec(rec)) != RM_EOF) {
    ...
    //        VLOG(2) << attributes[i].attrName << " " <<
    //        attributes[i].attrSpecs << " " << attributes[i].nullableIndex;
    ++i;
  }
  TRY(scan.CloseScan());
  if (i != attrCount)
    return SM_CATALOG_CORRUPT;

  //排序
  if (sort) {
    std::sort(attributes.begin(), attributes.end(),
              [](const DataAttrInfo &a, const DataAttrInfo &b) {
    ...
    for (int i = 0; i < attrCount; ++i) {
      ...
      if (p == q) {
        if (attributes[i].attrSpecs & ATTR_SPEC_NOTNULL) {
          std::cerr << cnt + 1 << ":" << p << " "
                    << "null value for non-nullable attribute" << std::endl;
          return SM_FILE_FORMAT_INCORRECT;
        }
...
/**
 2.打印有关指定关系中所有属性的信息,
 打印的信息包括：relName,attributeName,偏移,属性类型,属性长度,索引号
 输入参数：relName给定关系名
*/
...
RC SM_Manager::Set(const char *paramName, const char *value) { return 0; }

RC SM_Manager::Help() { return 0; }
